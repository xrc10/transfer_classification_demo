function eval = funBoltNBTransLearn(simM, lang)

useClassPriorTransProb = false;
useParamAdaptation = false;
...
if ~exist('dataSplit_bolt', 'dir')
    mkdir('dataSplit_bolt');
end

if ~exist('NBmodels_bolt', 'dir')
    mkdir('NBmodels_bolt');
end

%% load or create src data split
if ~exist(fullfile('dataSplit_bolt', ['english_', num2str(kFold), '_tf',  '.mat']), 'file')
    srcFolder = '/clair/ruochenx/LORELEI/data/Hausa/genre/english_tf';
    [XAll, dataSplit] = kFoldRcvSplit(srcFolder, kFold);
    save(fullfile('dataSplit_bolt', ['english_', num2str(kFold), '_tf']), 'XAll', 'dataSplit');
else
    load(fullfile('dataSplit_bolt', ['english_', num2str(kFold), '_tf', '.mat']));
end
...
if ~exist(fullfile('dataSplit_bolt', [lang, '_', num2str(kFold), '_tf',  '.mat']), 'file')
    
    tgtFolder = fullfile('/clair/ruochenx/LORELEI/data/Hausa/genre', [lang, '_tf']);
    
    [XAll, dataSplit] = kFoldRcvSplit(tgtFolder, kFold);
    
    save(fullfile('dataSplit_bolt', [lang, '_', num2str(kFold), '_tf']), 'XAll', 'dataSplit');
else
    load(fullfile('dataSplit_bolt', [lang, '_', num2str(kFold), '_tf', '.mat']));
end
...
for i = 1:kFold
    ...
    if ~exist(fullfile('NBmodels_bolt', ['english', '_', num2str(i), '.mat']), 'file')
        ...
        save(fullfile('NBmodels_bolt', ['english', '_', num2str(i)]), 'models');
    else
        fprintf('<<Src>>Getting previous model...\n');
        load(fullfile('NBmodels_bolt', ['english', '_', num2str(i), '.mat']));
    end

    %% test on src val set and test set
    if ~exist(fullfile('NBmodels_bolt', ['english_', lang, '_eval', '.mat']), 'file')
        ...
        load(fullfile('NBmodels_bolt', ['english_', lang, '_eval', '.mat']));
    end
    ...
    for k = 1:size(tgtClassNames,2)
        ...
        if useParamAdaptation
            %% get training data information
            foldData = srcDataSplit(className);
            tempPosIdx = foldData{1};
            tempNegIdx = foldData{2};
            trainCount = size(cell2mat(tempPosIdx(trnFolds)), 1) + size(cell2mat(tempNegIdx(trnFolds)), 1);
            XTgtAllExt = extSparseDim(XTgtAll, 2, pTgt);
            modTgt = infParamsOnUnlabel(modTgt, XTgtAllExt, trainCount);
        end
        %% val set
    ...
    if ~exist(fullfile('NBmodels_bolt', [lang, '_', num2str(i), '.mat']), 'file')
        models = containers.Map;
        for k = 1:size(tgtClassNames,2)
            className = tgtClassNames{k};
            foldData = tgtDataSplit(className);
        ...
        save(fullfile('NBmodels_bolt', [lang, '_', num2str(i)]), 'models');
    else
        fprintf('Getting previous model...\n');
        load(fullfile('NBmodels_bolt', [lang, '_', num2str(i), '.mat']));
    end
    
    if ~exist(fullfile('NBmodels_bolt', [lang, '_monoling_eval', '.mat']), 'file')
        ...
        load(fullfile('NBmodels_bolt', [lang, '_monoling_eval', '.mat']));
    end
...
if ~exist(fullfile('NBmodels_bolt', ['english_', lang, '_eval', '.mat']), 'file')
    save(fullfile('NBmodels_bolt', ['english_', lang, '_eval']), 'avgSrcMacroF1', 'avgSrcMicroF1');
end

if ~exist(fullfile('NBmodels_bolt', [lang, '_monoling_eval', '.mat']), 'file')
    save(fullfile('NBmodels_bolt', [lang, '_monoling_eval']), 'avgTgtMacroF1', 'avgTgtMicroF1');
end
